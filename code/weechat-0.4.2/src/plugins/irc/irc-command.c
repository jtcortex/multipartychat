...
int 
irc_command_mpotr (void *data, struct t_gui_buffer *buffer, int argc,
		   char **argv, char **argv_eol)
{
    struct sockaddr_in addr;
    socklen_t length;
	...
	if (weechat_strcasecmp (argv[1], "auth") == 0)
	{
	    infolist = weechat_infolist_new ();
	    if (infolist)
	    {
	        ...
	        if (item)
		{
		    weechat_infolist_new_var_string (item, "plugin_name", weechat_plugin->name);
                    ...
                    snprintf (str_address, sizeof (str_address),
                              "%lu", address);
                    weechat_infolist_new_var_string (item, "local_address", str_address);
		    weechat_hook_signal_send ("mpotr_auth",
					      WEECHAT_HOOK_SIGNAL_POINTER,
					      infolist);
		}
		weechat_infolist_free (infolist);
	...
	else
	{
	    weechat_printf (ptr_server->buffer,
			    _("%s%s: wrong arguments for \"%s\" command"), 
			    weechat_prefix ("error"), IRC_PLUGIN_NAME, "mpotr");
	    return WEECHAT_RC_OK;
	}
	return WEECHAT_RC_OK;
    }
...
irc_command_init ()
{
    ...
    weechat_hook_command ("mpotr",
			  N_("begin an mpOTR session"), 
			  N_("[-auth]"),
                          N_("auth: authenticate everyone in channel"),
                          NULL, &irc_command_mpotr, NULL);
